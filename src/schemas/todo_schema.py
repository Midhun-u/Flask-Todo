...
class Todos(SQLModel, table=True):
    ...
    user_id: str = Field(allow_mutation=False, nullable=False, max_length=150)
    task: str = Field(nullable=False, min_length=5, max_length=100)
    is_completed: bool = Field(default=False, nullable=False)
    created_at: str | None = Field(default=datetime.now().isoformat(), nullable=False, allow_mutation=False)